...
def _simple_config() -> AppConfig:
    return AppConfig(
        categories=[
            ...
            CategoryConfig(
                name="system", color="bright_black", match_type="user", content_contains=["<system-reminder>"]
            ),
        ]
    )
        ...
        [
            {
                ...
                "message": {
                    "content": [
                        {"type": "text", "text": "Here is a <system-reminder>important</system-reminder> note."}
                    ],
                },
            },
        ],
...
def test_label_propagates_from_config_to_category_tokens(tmp_path: Path) -> None:
    ...
    config = AppConfig(
        categories=[
            CategoryConfig(
                name="claude", color="green", label="Cl", match_type="assistant", match_content_types=["text"]
            ),
        ]
    )
    result = parse_transcript(str(path), config)
